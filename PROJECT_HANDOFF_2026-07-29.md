...
_Last updated: 2026-07-31 (API deploy pipeline restored + backend LIVE). Branch: `preview`._
...
- **✅ BACKEND DEPLOYED (2026-07-31).** Investigation revealed the Azure SWA deploy pipeline had been **dead since 2026-07-21**: commit `e296bef` ("Remove Azure SWA workflow from preview repo") reached origin/main via cross-merges, deregistering the only deploy path — API changes since then (`7553ca5` analytics-loss fix, `f0188b3` beacon auth, `vsPromoSeen`) never reached Azure. Fixed in two commits: `03bfdc5` restores the last-working workflow (`1843eb1` content: checkout@v4, APP_CLIENT_KEY env guard; LF endings — CRLF breaks workflow loading; plus a `github.repository == 'vpietri-stack/Classroom-survivors'` guard so it never runs on the preview repo, preserving e296bef's intent); `6d97e59` fixes the resulting "app content too large" failure (repo media outgrew the SWA Free tier's 250MB app limit) by staging a MINIMAL app dir (`swa_app/`: staticwebapp.config.json + injected app-config.json + stub index, `skip_app_build`) since the SWA only exists to host `/api` — the game is served by GitHub Pages. **Run 30620535549 succeeded; live API verified** (login → 401 + correct CORS = Functions + Cosmos up; `vsPromoSeen` whitelist now live → promo cross-device persistence fully working). Deploy path going forward: any push to origin/main auto-deploys the API. NOTE: `app-config.json` is gitignored + 404 on Pages, so the frontend has ALWAYS run the empty-key fallback; the key injection now lives in swa_app (unchanged behavior). The full preview line (VS revamp + character select + Ruler/Jump Rope + HiDPI on all 3 games + iOS/WeChat audio fixes + split SFX/music mute + HUD-timer fix + new intro + one-time VS promo) is now LIVE at https://vpietri-stack.github.io/Classroom-survivors/. This **reversed the old “VS must never go to origin/main” rule** (VS was in-dev then; user explicitly shipped it). Method: branches had DIVERGED — origin/main held 3 production-only commits NOT on preview (`8c87a58` speech PASS-screen UI, `1443d33` OS-census telemetry+`api/analyze_devices.js`, `5a8616b` scramble variant) plus cherry-pick duplicates of preview fixes. A force-push would have DESTROYED those 3, so instead: `git checkout main; git merge preview` (1 trivial conflict = game.js cache-bust string, kept preview's), verified prod-only content preserved + full suite green (manifest, 12×3 device matrix, input+uno, timer, promo, boss, charselect), pushed origin/main, then fast-forwarded preview to match. **Both branches are now identical at `edf0577` — divergence resolved, future deploys are clean.** SCM memories still say “VS excluded / cherry-pick only” — those are now OBSOLETE; the project ships full preview→origin/main.
