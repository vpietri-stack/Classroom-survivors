...
// --- GAME DATA ---
const POWER_UPS = [
    { id: 'whip', name: "Magic Whip", icon: "­ƒ¬ä", type: "weapon", desc: "Front Attack" },
    { id: 'wand', name: "Spirit Wand", icon: "Ô£¿", type: "weapon", desc: "Fires at nearest enemy" },
    { id: 'orb', name: "Orbit", icon: "­ƒö«", type: "weapon", desc: "Spins around you" },
    { id: 'axe', name: "Axe", icon: "­ƒ¬ô", type: "weapon", desc: "Add one more axe" },
    { id: 'cross', name: "Cross", icon: "Ô£Ø´©Å", type: "weapon", desc: "Boomerang effect" },
    { id: 'water', name: "Santa Water", icon: "­ƒÆº", type: "weapon", desc: "Drops damaging puddle" },
    { id: 'knife', name: "Knife", icon: "­ƒö¬", type: "weapon", desc: "Fires in facing direction" },
    { id: 'speed', name: "Swiftness", icon: "­ƒæƒ", type: "stat", desc: "+10% Move Speed" },
    { id: 'might', name: "Spinach", icon: "­ƒÑ¼", type: "stat", desc: "+10% Damage" }
];
...
class MainScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Player
        this.player = this.add.text(0, 0, '­ƒºÖÔÇìÔÖé´©Å', { fontSize: '50px', padding: { top: 10 } }).setOrigin(0.5);
                ...
                if (t.fireballs) {
                    t.fireballs.forEach(fb => {
                        ...
                        if (this.gameTime % 2 === 0) {
                            const trail = this.add.text(fb.x, fb.y, '­ƒöÑ', { fontSize: '20px' }).setOrigin(0.5).setAlpha(0.5);
    ...
    spawnEnemy(distance = null) {
        ...
        const isBat = type === 1;
        let sprite = isBat ? '­ƒªç' : (type === 2 ? '­ƒºƒ' : '­ƒæ¥');
    ...
    spawnBoss() {
        const boss = this.add.text(this.player.x, this.player.y - 600, '­ƒæ╣', { fontSize: '80px', padding: { top: 20 } }).setOrigin(0.5);
    ...
    spawnSingleObstacle(distance = null) {
        const obstacleTypes = [
            { emoji: '­ƒî▓', fontSize: '100px', bodyRad: 10, isTree: true },
            { emoji: '­ƒî│', fontSize: '100px', bodyRad: 10, isTree: true },
            { emoji: '­ƒ¬¿', fontSize: '50px', bodyRad: 20 },
            { emoji: '­ƒî┐', fontSize: '50px', bodyRad: 20 },
            { emoji: '­ƒøû', fontSize: '150px', bodyRad: 65 }
        ];
    ...
    spawnBatSwarm() {
        ...
        for (let i = 0; i < count; i++) {
            this.time.delayedCall(i * 40, () => {
                ...
                const bat = this.add.text(startX + ox, startY + oy, '­ƒªç', { fontSize: '20px' }).setOrigin(0.5);
            ...
            if (w.type === 'orb') {
                ...
                if (w.sprites.length !== w.level) {
                    ...
                    for (let i = 0; i < w.level; i++) {
                        const orb = this.add.text(0, 0, '­ƒö«', { fontSize: '20px' }).setOrigin(0.5);
    ...
    fireAxe(w) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const axe = this.add.text(this.player.x, this.player.y, '­ƒ¬ô', { fontSize: '24px' }).setOrigin(0.5);
    ...
    fireCross(w) {
        synthShoot('cross');
        const cross = this.add.text(this.player.x, this.player.y, 'Ô£Ø´©Å', { fontSize: '24px' }).setOrigin(0.5);
    ...
    fireKnife(w) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const knife = this.add.text(this.player.x, this.player.y, '­ƒö¬', { fontSize: '24px' }).setOrigin(0.5);
    ...
    fireSantaWater(w) {
        ...
        const bottle = this.add.text(tx, ty - 500, '­ƒº¬', { fontSize: '30px' }).setOrigin(0.5);
    ...
    spawnLootbox(x, y) {
        ...
        const specials = [
            { id: 'heart', icon: 'ÔØñ´©Å', type: 'special' },
            { id: 'vortex', icon: '­ƒîÇ', type: 'special' },
            { id: 'tornado', icon: '­ƒî¬´©Å', type: 'special' }
        ];
    ...
    spawnTornado() {
        ...
        for (let i = 0; i < 12; i++) {
            const fb = this.add.text(this.player.x, this.player.y, '­ƒöÑ', { fontSize: '40px' }).setOrigin(0.5);
